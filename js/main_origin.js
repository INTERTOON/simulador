...
// 	        
	        ...
	        // {
	        	// formFree = false;
// 	        	
	        	// var urlCall = "http://www.biancogres.com.br/simulador/actions.php";
	        	// var data = "";
	        	// data += "email="+$("#formEmail #email").val();
	        	// // data += "&image="+$("#formEmail #image").val();
				// $.ajax({url:urlCall,
					// dataType:'html',
					// type:"POST",
					// data: data,
					...
					// success:function(response){
						...
						// if(response.status){
							// if(response.status || response.indexOf( "1PHP Warning" ) != -1)
							// {
								// console.log('done');
            					// $('#emaill').fadeOut(500);
            					// $("#formEmail #email").val('');
...
// 	        	
		...
		// }
	// });
     // $('#guardar').on("touchend", function(e){
		// mainStage.toDataURL({
          // callback: function(dataUrl) {
            // /*
             // * here you can do anything you like with the data url.
...
function parseJSONData(data)
{
	...
	for(var i=0; i<result; i++)
	{
		...
		item += '<img class="loading" alt="loading" src="css/images/ajax-loader.gif" />';
...
function prepareDataForLoading(){
	...
	for(var i = 0; i<stuff; i++)
	{
		...
		for(var j = 0; j<surfacesTotal; j++)
		{
			...
			for(var k = 0; k<tilesTotal; k++)
			{
				tiles.push(new ConfigObject(k,JSONData[i].surfaces[j].tiles[k].tileImage,JSONData[i].surfaces[j].tiles[k].defaultImage,JSONData[i].surfaces[j].tiles[k].title));
				ambienteImgs+=2;
				// totalImagesToLoad+=2;
			}
...
function configStage(){
	...
	// $('#tiles').on("swipeleft", function(e){
		// swipe = true;
		// console.log('left');
	// });
// 	
	// $('#tiles').on("MSPointerMove", function(e){
		// swipe = true;
		// console.log('move');
	// });
// 	
	// $('#tiles').on( "swiperight", function(e){
		// swipe = true;
		// console.log('right');
	// });
}
...
// 
	...
	imageObj.src = 'http://www.biancogres.com.br/simulador/img/site/biancogres-small-logo.png';
